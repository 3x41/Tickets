...
$stmt = $pdo->prepare('SELECT * FROM priority WHERE id = ?');
...
if (isset($_GET['status']) ) {
	...
	if ($_GET['status']=="delete"){
		//echo ("DELETED");
		$stmt = $pdo->prepare('DELETE FROM priority WHERE id = ?');
		$stmt->execute([ $_GET['id'] ]);
...
$stmt = $pdo->prepare('SELECT * FROM priority WHERE id = ? ORDER BY created DESC');
$stmt->execute([ $_GET['id'] ]);
...
<div class="content view">
	...
	<a href="view_priority.php?id=<?=$_GET['id']?>&status=delete" class="btn red">Delete</a>
	</div>
	...
	<?=htmlspecialchars($ticket['priority'], ENT_QUOTES)?>
	</h3>
